...
# of snakey's head,  Background is now black.  Food is an apple.
# 6. resizeable window
...
def updateMonsters(fully_random=False):
    ...
    for i in range(len(monsters)):
        (mx, my) = monsters[i]
        d = line_length / 2
        possibleMoves = [(0, -d), (d, 0), (0, d), (-d, 0)]  # [N,E,S,W]
        # Randomly choose ONE of the possibleMoves
        move = possibleMoves[random.randint(0, 3)]
        # Are we using a full_random approach?
        if fully_random:
            # YES => update monster with that random direction
            monsters[i] = (mx + move[0], my + move[1])
        else:
            # NO => only move direction if it moves us closer to
            # food.  If random move doesn't move us closer to food
            # we only move 25% of time.
            fx, fy = food
            fxgap = abs(mx - fx)
            fygap = abs(my - fy)
            if fxgap > foodwidth and abs(mx + move[0] - fx) < fxgap:
                ...
                # 25% of the time just move anyway - adds some noise
                if random.randint(0, 3) == 0:
                    monsters[i] = (mx + move[0], my + move[1])
    # We are going to add monsters as we get longer!
    if snakelen > 140 and len(monsters) == 4:
        createMonster()
    if snakelen > 200 and len(monsters) == 5:
        createMonster()
...
def drawFood():
    ...
    # create new sprite on canvas - its center on the canvas is at (fx, fy)
    foodSprite = canvas.create_image(fx, fy, image=foodImage, anchor=CENTER)
...
window = Tk()
window.title("MySnakey9")
canvas = Canvas(bg=bgcolour, height=H, width=W, highlightthickness=0)
foodImage = PhotoImage(file='strawberry6.gif')
