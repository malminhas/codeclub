...
def updateCounter():
    ...
    offset = 65
    canvas.create_rectangle(5+offset, 5, 25+offset, 25,
                            outline=scoreColour, fill=bgcolour)
    canvas.create_rectangle(W-25, 5, W-5, 25, outline="green", fill=bgcolour)
    ...
    canvas.create_text(W-15, 15, text="%d" % highestScore,
                       font=("Purisa", 16), fill="green")
...
window = Tk()
window.title("MySnakey5")
canvas = Canvas(bg=bgcolour, height=H, width=W, highlightthickness=0)
canvas.pack()
slider = Scale(window, length=360, width_=10, from_=10,
               to=100, orient=HORIZONTAL, resolution=10)
slider.set(30)
slider.pack()
